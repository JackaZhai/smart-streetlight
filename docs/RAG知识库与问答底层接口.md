...
本次将原有硬编码 FAQ 问答升级为本地 RAG 知识库检索，不依赖外部大模型、MaxKB 或云端向量数据库，保证本地开发、Spark 服务器和 Docker Compose 环境均可运行。

- 新增本地知识库数据文件，覆盖 7 类运维知识。
- 新增知识库检索模块，支持关键词、同义词和设备 ID 识别。
- 问答接口结合知识库命中片段、设备状态和故障预判生成回答。
- `/api/agent/chat` 保持原有 `answer`、`references` 字段，并扩展 `matches`、`suggestedActions`。
- 前端智能问答展示回答正文、引用来源、知识命中片段和推荐操作。
...
| 后端相关测试 | `npm --workspace backend test -- streetlight.test.ts knowledgeBase.test.ts api.test.ts` | 已通过，`13 passed` |
